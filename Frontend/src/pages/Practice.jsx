...
function Practice() {
  ...
  const handleFilterChange = (e) => {
    ...
    if (selectedValue === "mastered") {
      filteredCards = filteredCards.filter(card => card.status === "mastered");
    } else if (selectedValue === "need practice") {
      filteredCards = filteredCards.filter(card => card.status === "need practice");
    } else if (selectedValue === "not studied") {
      filteredCards = filteredCards.filter(card => card.status === "not studied");
    } else if (selectedValue === "not studied and need practice") {
      filteredCards = filteredCards.filter(card => card.status === "need practice" || card.status === "not studied");
    }
          ...
          {currentCardsSet?.length > 0 && (
            <>
              <div key={currentStudySet._id}>
                ...
                <p className={filterStatus === "all" ? "block" : "hidden"}>Status: {currentCard?.status}</p>
                ...
                <div className='my-5 ml-[110px] flex gap-[10px]'>
                  <button onClick={() => handlePreviousCard(currentCardsSet)} className='bg-gray-400 p-[10px] rounded-md'>Previous</button>
                  <p>{currentIndex + 1} / {currentCardsSet?.length}</p>
                  <button onClick={() => handleNextCard(currentCardsSet)} className='bg-gray-400 mx-10 p-[10px] rounded-md'>Next</button>
                </div>
              </div>
            </>
          )}
